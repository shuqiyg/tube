...
async function processInChuncks<T,U> (
    items: T[],
    chunkSize: number,
    processItem: (items: T) => Promise<U>
): Promise<U[]> {
    const results: U[] = [];
...
async function main() {
    ...
    await processInChuncks(users, 1, (user) => 
        prisma.user.upsert({
            where: {
                id: user.id
            },
            //background or backgroundImage in prisma schema?????
            update: {
                ...user,
    ...
    await processInChuncks(videos, 1, (video) => 
        prisma.video.upsert({
            where: { id:video.id },
            update: {
                ...video,
    ...
    await processInChuncks(videoEngagements, 1, (videoEngagement) =>
        prisma.videoEngagement.create({ data: videoEngagement })
    );

    await processInChuncks(followEngagements, 1, async(followEngagement) => {
    ...
    await processInChuncks(announcements, 1, (announcement) => 
        prisma.announcement.create({data: announcement })
    );

    await processInChuncks(
    ...
    await processInChuncks(comments, 1, (comment) => prisma.comment.upsert({
        where: { id: comment.id},
        update: {
            ...comment,
    ...
    await processInChuncks(playlists, 1, async (playlist) => prisma.playlist.upsert({
    ...
    await processInChuncks(playlistHasVideos, 1, (playlistHasVideo) =>
